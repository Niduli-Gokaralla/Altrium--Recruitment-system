...
// Real candidate count for a job, computed from actual candidate
// records — no fabricated placeholder numbers.
function realCandidateCount(jobId) {
  return allCandidates.filter((c) => c.jobOpeningId === jobId).length;
}
...
async function loadJobs() {
  ...
  try {
    const [jobsResponse, candidatesResponse] = await Promise.all([
      fetch(API_BASE_URL, { headers: { ...getAuthHeader() } }),
      fetch(CANDIDATES_API, { headers: { ...getAuthHeader() } }),
    ]);
    if (!jobsResponse.ok) throw new Error("Failed to load job openings");
    allJobs = await jobsResponse.json();
    allCandidates = candidatesResponse.ok ? await candidatesResponse.json() : [];
...
function renderTable(jobs) {
  ...
  jobs.forEach((job) => {
    ...
    tr.innerHTML = `
      ...
      <td class="candidate-count">${realCandidateCount(job.id)}</td>
      ...
      <td class="text-end">
        ...
        ${job.status !== "CLOSED" ? `<button class="action-btn text-warning" title="Close Job" data-action="close" data-id="${job.id}"><i class="bi bi-x-circle"></i></button>` : ""}
...
document.getElementById("jobsTableBody").addEventListener("click", (e) => {
  ...
  if (action === "close") closeJobDirectly(id);
});

async function closeJobDirectly(id) {
  const job = allJobs.find((j) => j.id === id);
  if (!job) return;
  if (!confirm(`Close "${job.title}"? It will no longer be marked as an open position.`)) return;
...
function openEditForm(id) {
  ...
  document.getElementById("jobCutoffScore").value = job.cutoffScore || 60;
...
jobForm.addEventListener("submit", async (e) => {
  ...
  const payload = {
    ...
    cutoffScore: Number(document.getElementById("jobCutoffScore").value) || 60,
...
function openDetails(id) {
  ...
  const jobCandidates = allCandidates.filter((c) => c.jobOpeningId === job.id);
  const pipelineStages = ["APPLIED", "SCREENING", "SHORTLISTED", "INTERVIEW", "HIRED", "REJECTED"];
  const pipelineCounts = pipelineStages.map((stage) => jobCandidates.filter((c) => c.stage === stage).length);
  ...
  document.getElementById("detailsBody").innerHTML = `
    ...
    <div class="details-section">
      ...
      <div class="pipeline-mini">
        ${pipelineStages.map((stage, idx) => `<div><div class="num">${pipelineCounts[idx]}</div><div class="label">${STAGE_LABELS[stage]}</div></div>`).join("")}
      </div>
    </div>

    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mt-4">
      <a href="candidates.html" class="btn btn-outline-secondary btn-sm">
