  ...
  try {
    ...
    if (!data.upcomingInterviews || data.upcomingInterviews.length === 0) {
      ...
      upcomingList.innerHTML = data.upcomingInterviews.map((i) => {
        ...
        return `
        <li class="int-upcoming-item">
          <div class="int-date-badge">
            <span class="int-date-day">${day}</span>
            <span class="int-date-month">${month}</span>
          </div>
          <div>
            <div class="int-upcoming-title">${escapeHtml(i.candidateName)} — ${escapeHtml(i.stage)}</div>
            <div class="int-upcoming-meta">${escapeHtml(i.jobOpeningTitle)} · ${formatTime(i.interviewTime)}</div>
          </div>
        </li>
      `;
      }).join("");
