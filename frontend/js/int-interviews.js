...
const STAGE_DECISION_LABELS = { ADVANCE: "Advanced", REJECT: "Rejected", ON_HOLD: "On Hold" };
const STAGE_DECISION_BADGE_CLASS = { ADVANCE: "bg-success", REJECT: "bg-danger", ON_HOLD: "bg-warning text-dark" };

function stageDecisionBadge(decision) {
  if (!decision) return `<span class="text-muted small">Pending</span>`;
  return `<span class="badge ${STAGE_DECISION_BADGE_CLASS[decision]}">${STAGE_DECISION_LABELS[decision]}</span>`;
}
...
function renderTable(interviews) {
  ...
  tbody.innerHTML = interviews.map((i) => `
    <tr>
      ...
      <td>${stageDecisionBadge(i.stageDecision)}</td>
...
async function openDetails(id) {
  ...
  // AC (interviewer permissions): visible read-only, no way to change
  // it — Interviewers can see the Hiring Manager's stage decision but
  // never make or edit one themselves.
  const stageDecisionRow = `
    <div class="details-meta-row"><span>Stage Decision</span><span>${stageDecisionBadge(i.stageDecision)}</span></div>
    ${i.stageDecisionComment ? `<div class="details-meta-row"><span>Decision Comment</span><span>${escapeHtml(i.stageDecisionComment)}</span></div>` : ""}
  `;
