...
public class InterviewerController {
    ...
    // Interviewer only ever sees THEIR role's feedback row — never HR's
    // or the Hiring Manager's row for the same interview.
    ...
    public ResponseEntity<FeedbackResponse> getFeedback(@PathVariable Long id, Authentication authentication) {
        ...
        return ResponseEntity.ok(interviewService.getFeedbackForRole(id, "INTERVIEWER"));
    }

    // Submits/updates the Interviewer's own feedback row only.
    ...
    public ResponseEntity<?> submitFeedback(@PathVariable Long id, @Valid @RequestBody FeedbackRequest request, Authentication authentication) {
        assertInterviewInScope(id, authentication.getName());
        return ResponseEntity.ok(interviewService.submitFeedback(id, request, authentication.getName(), "INTERVIEWER"));
    }
